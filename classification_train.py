...
def classification_train():
    ...
    # 데이터에서 각 인자를 추출
    input_learning_rate = input_data.get('learningRate', None)
    input_epochs = input_data.get('Epochs', None)
    input_patience = input_data.get('Patience', None)
    ...
    return jsonify({'f1Score': f1, 'f1ScoreM': f1_micro, 'f1ScoreW': f1_weighted, 'auc': np.mean(roc_auc)})
